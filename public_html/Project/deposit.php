...
<?php
 ...
 $results = [];
 $uid = get_user_id();
 $stmt = $db->prepare("SELECT account_number FROM Accounts WHERE user_id = $uid AND account_type != 'Loan' AND is_active = 1 AND frozen=False");
 $stmt->execute();
...
if((isset($_POST['account']) && $_POST['account'] != "Account") && !$hasError){
 ...
 $memo = strval($memo);
 $results = [];
 $stmt = $db->prepare("SELECT id FROM Accounts WHERE account_number=$useracc LIMIT 1");
 try {
     $stmt->execute();
 ...
 $balstmt->execute();
 $balstmt = $db->prepare("UPDATE Accounts SET balance=(balance+$Ubalchange) WHERE account_number=$useracc");
 $balstmt->execute();
